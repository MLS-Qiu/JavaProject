$(function () {
    ...
    $(".insert-bnt").click(function () {
        location.href = jCtx + "customer/cusInsert";
    });
    ...
    /*//导出按钮事件
    $(".excelOut-bnt").click(function () {
        //获得选择器的状态
        let $chs = $("#tbl>tbody>tr>td:first-child>:checked");
        if ($chs.length === 0) {
            layer.msg("请选中您要导出的记录");
            return;
        } else {
            let ids = [];
            //将选中的数据的id取出来放进ids数组中
...
function search(pageNum, pageSize) {
    let cusId = $(".search #c_id").val();
    let cusName = $(".search #c_name").val();
    let cusSex = $(".search #c_sex").val();
    let cusAge = $(".search #c_age").val();
    let cusPhone = $(".search #c_phone").val();
    let cusEmail = $(".search #c_email").val();
    let cusBirthFrom = $(".search #c_birthdayFrom").val();
    let cusBirthTo = $(".search #c_birthdayTo").val();
    let cusAddress = $(".search #c_address").val();
    let cusNote = $(".search #c_note").val();
    ...
    if (cusBirthFrom.trim() !== " ") {
        data.cusBirthFrom = cusBirthFrom;
    }
    if (cusBirthFrom.trim() !== " ") {
        data.cusBirthTo = cusBirthTo;
    }
    if (cusAddress.trim() !== " ") {
        data.cusAddress = cusAddress;
    }
    if (cusNote.trim() !== " ") {
        data.cusNote = cusNote;
    }

    let url = jCtx + "customer/cueList";

    //发送一个ajax请求
    $.post(url, data, function (resp) {
        let cus = resp.cusList;
        let pg = resp.pi;
        ...
        cus.forEach(function (i) {
            let $tr = $("<tr>");
            /*$tr.append($("<td><input class='checkChild' type='checkbox'></td>"));*/
            $tr.append($("<td>" + i.cusId + "</td>"));
            $tr.append($("<td>" + i.cusName + "</td>"));
            $tr.append($("<td>" + i.cusSex + "</td>"));
            $tr.append($("<td>" + i.cusAge + "</td>"));
            $tr.append($("<td>" + i.cusPhone + "</td>"));
            $tr.append($("<td>" + i.cusEmail + "</td>"));
            $tr.append($("<td>" + i.cusBirthday + "</td>"));
            $tr.append($("<td>" + i.cusAddress + "</td>"));
...
function deleteById(ids) {
    console.log(ids);
    let url = jCtx + "customer/cusDelete";
...
function excelOut(ids){
    console.log(ids);
    let url = jCtx + "customer/cusExcOut";
    ...
    $.ajax(url, {
        ...
        success: function (resp) {//成功调用此函数
            if (resp.success) {
                layer.msg(resp.suc || "导出信息成功");
                /*location.reload();//刷新页面*/
            } else {
                layer.alert(resp.ero || "导出信息失败");
            }
        },
        error: function () {//失败调用此函数
            layer.alert(resp.ero || "导出信息失败");
